...
const DatVe = () => {
  ...
  const handleContinue = () => {
    const selectedTicketTypes = ticketTypes
      .filter(ticketType => ticketQuantities[ticketType.id] > 0) // Lọc loại vé đã chọn
      .map(ticketType => ({
        name: ticketType.TenVe,
        price: ticketType.GiaVe,
        quantity: ticketQuantities[ticketType.id] // Số lượng vé đã chọn
      }));
    
    // Tìm thông tin showtime và room tương ứng với ID đã chọn
    ...
    const holdTimeInMinutes = 5; // Thay đổi giá trị này nếu cần
    ...
    Cookies.set("bookingInfo", JSON.stringify({
      ...
      ticketQuantities,    // Số lượng vé
      comboQuantities,     // Số lượng combo
      totalAmount,         // Tổng số tiền
      movieName: movies.Ten,   // Tên phim
      showtime: selectedShowtime ? selectedShowtime.NgayChieu : null, // Thời gian chiếu
      room: selectedRoom ? selectedRoom.TenPhongChieu : null, // Số phòng
      ticketTypes: selectedTicketTypes,
      holdTime   
    }), { expires: 1 / 288 }); // Hết hạn sau 5 phút (1 ngày / 288)
  
    // Chuyển hướng hoặc thực hiện hành động khác sau khi lưu vào cookie
