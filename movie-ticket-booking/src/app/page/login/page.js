...
const Login = () => {
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting, setFieldError }) => {
            try {
                const res = await fetch('http://localhost:3000/users/login', { // Chỉnh sửa endpoint API để có thể dùng cho cả users và employees
                    method: 'POST',
                ...
                if (payload.IsAdmin === 1) { // Kiểm tra vai trò người dùng
                    window.location.href = 'http://localhost:3001'; // Chuyển đến trang admin (user)
                } else if (payload.IsAdmin === 0) {
                    window.location.href = 'http://localhost:3002'; // Chuyển đến trang nhân viên (employee)
                }
            } catch (error) {
