        ...
        onSubmit: async (values, { setSubmitting, setFieldError }) => {
            try {
                const res = await fetch('http://localhost:3000/users/login', {
                    method: 'POST', headers: { 'Content-Type': 'application/json', }, body: JSON.stringify({ usernameOrEmail: values.usernameOrEmail, MatKhau: values.Matkhau }),
                });
    ...
    return (
        <div className="flex justify-center items-center bg-cover bg-center w-full min-h-screen bg-[url('../../public/images/background.png')]">
            <form onSubmit={formik.handleSubmit} className="flex flex-col justify-center items-center p-6 sm:p-8 md:p-10 rounded-lg text-white w-[90%] sm:w-[85%] md:w-[750px] lg:w-[900px] h-auto" style={{ backgroundColor: 'rgba(0, 0, 0, 0.7)' }}  >
                ...
                <input type="text" id="usernameOrEmail" name="usernameOrEmail" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.usernameOrEmail} placeholder="Tên đăng nhập hoặc email" required className={`w-full md:w-[520px] h-[40px] sm:h-[45px] p-2 mb-3 border-2 rounded-md text-sm sm:text-base ${formik.touched.usernameOrEmail && formik.errors.usernameOrEmail ? 'border-red-500' : 'border-white'} bg-[#212529] placeholder-white placeholder-opacity-50`} /> {formik.touched.usernameOrEmail && formik.errors.usernameOrEmail && <p className="text-red-500">{formik.errors.usernameOrEmail}</p>}
                ...
                <input type="password" id="MatKhau" name="Matkhau" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.Matkhau} placeholder="Mật khẩu" required className={`w-full md:w-[520px] h-[40px] sm:h-[45px] p-2 mb-3 border-2 rounded-md text-sm sm:text-base ${formik.touched.Matkhau && formik.errors.Matkhau ? 'border-red-500' : 'border-white'} bg-[#212529] placeholder-white placeholder-opacity-50`} />  {formik.touched.MatKhau && formik.errors.Matkhau && <p className="text-red-500">{formik.errors.Matkhau}</p>}
                ...
                <div className="flex justify-center items-center text-xs sm:text-sm">
                    <p>Bạn chưa có tài khoản?</p> <Link href="/page/register" className="text-[#F5CF49] ml-1">Đăng ký ngay!</Link>
                </div>
            </form>
        </div>
    );
