...
const TheLoai = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/theloai/delete/${id}`, { // Ensure endpoint is correct
        method: "DELETE",
      });
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/theloai/edit/${editedCategory._id}`, {
        method: "PUT",
        headers: {
